...
### 1. [Unstructured Information Extraction Workflow](./unstructured-ie)
...
### 2. [Parse-Translate-Classify Workflow](./parse-translate-classify)
...
### 3. [Unstructured Knowledge Base Workflow](./knowledge-base)
...
### 4. [Information Extraction — Selected Pages](./ie-selected-pages)

A variant of the IE workflow that adds page-level classification before extraction:
- **Parse** documents with `ai_parse_document`
- **Extract** page-level content
- **Classify** individual pages to identify relevant sections
- **Extract** structured information from selected pages only

**Best for**: Large documents where only specific pages contain target information, reducing LLM cost by filtering irrelevant pages

**Key features**: Page-level classification, selective extraction, configurable page/classification/extraction tables
...
# Choose a workflow
cd unstructured-ie                    # Information extraction
# cd parse-translate-classify         # Parse-translate-classify
# cd knowledge-base                   # Knowledge base
